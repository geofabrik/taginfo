...
class Taginfo < Sinatra::Base
    ...
    api(4, 'tag/combinations', {
        ...
        return generate_result(@api, total,
            res.map do |row| {
                ...
                :to_fraction    => has_this_key ? (row['together_count'].to_f / has_this_key.to_f).round(4): nil,
    ...
    api(4, 'tag/overview', {
        ...
        data[:description] = {}
        @db.select("SELECT description, lang FROM wiki.wikipages WHERE key=? AND value=? AND description IS NOT NULL ORDER BY lang", key, value).execute.each do |row|
